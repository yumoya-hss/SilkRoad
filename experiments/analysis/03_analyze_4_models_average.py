...
INPUT_FILE = "dataset_optimal_filtered_v3.json" 
...
def calculate_filtered_average(file_path):
    ...
    for lang in sorted_langs:
        ...
        # 🔥 修改点：所有指标统一乘 100 转换为百分制
        qe_avg = np.mean(metrics['QE']) * 100 if metrics['QE'] else 0.0
        bs_avg = np.mean(metrics['BS']) * 100 if metrics['BS'] else 0.0
        # CLIP 原来是 0.28 现在变成 28.00
        clip_avg = np.mean(metrics['CLIP']) * 100 if metrics['CLIP'] else 0.0 
    ...
    rows.append({
        ...
        "Ours-CLIP": np.mean(global_stats['CLIP']) * 100 if global_stats['CLIP'] else 0.0, # 🔥 修改点
    ...
    print("\n" + "="*60)
    print("📊 Ours (Filtered) Dataset Quality Analysis (Scale: 0-100)")
    print("="*60)
