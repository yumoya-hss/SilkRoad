...
# 用户配置区域
# ==========================================
DEFAULT_LLAVA_PATH = "/mnt/raid/hss/model/llava-v1.6-mistral-7b-hf"
...
def clean_text(text):
    ...
    # 移除 Mistral/LLaVA 可能生成的特殊 Token 和 Prompt 残留
    if "[/INST]" in text:
        text = text.split("[/INST]")[-1]
    elif "INST]" in text: # 容错
        text = text.split("INST]")[-1]
    
    special_tokens = ["<s>", "</s>", "[INST]", "ASSISTANT:", "USER:"]
...
def rank_and_score(img_path, img_id, img_features_db, candidates, 
    ...
    """
    ✅ 综合评分函数：
    1. SigLIP: 用于排序 (Ranking)，决定哪个是 best_caption。
    2. CLIP: 用于评分 (Scoring)，计算 CLIPScore 作为指标。
    """
    ...
    try:
        ...
        inputs = clip_processor(
            ...
            padding=True,
            truncation=True,
            max_length=77 # CLIP 文本最大长度通常为 77
        ).to(device)
...
def main():
    parser = argparse.ArgumentParser(description="LLaVA-v1.6-Mistral Robust Fix with CLIPScore")
    ...
    # 3. 加载 LLaVA 模型
    print(f"[{args.gpu_id}] 加载 LLaVA ({args.llava_path})...", file=sys.stderr)
    ...
    try:
        ...
        llava_processor = LlavaNextProcessor.from_pretrained(args.llava_path, trust_remote_code=True)
        ...
        print(f"❌ 模型加载失败: {e}", file=sys.stderr); return
    ...
    # 6. 生成闭包
    def generate_batch(prompt_text, image_list, num_return):
        # 强制 Mistral Prompt 格式
        full_prompt = f"[INST] <image>\n{prompt_text} [/INST]"
        
        inputs = llava_processor(
            text=[full_prompt] * len(image_list), 
            images=image_list, 
            padding=True, 
            return_tensors="pt"
        ).to(device_str)
        ...
        with torch.inference_mode():
            max_new = 64 if "within 18 words" in prompt_text else 128
        ...
        raw_texts = llava_processor.batch_decode(generated_ids, skip_special_tokens=True)
        ...
        for item in tqdm(subset_data, desc=f"GPU{args.gpu_id}"):
            ...
            try:
                with Image.open(full_img_path) as img:
                    ...
                    p_short = "Describe the main objects in the image in English within 18 words. Focus strictly on visual attributes. Do not describe text or watermarks."
                    p_long = "Describe the image in detail in English within 45 words. Focus on appearance and actions. In a second sentence, describe spatial relationships. Ignore watermarks."

                    # 生成
                    short_cands = generate_batch(p_short, [raw_image], args.num_candidates)
                    long_cands = generate_batch(p_long, [raw_image], args.num_candidates)
            ...
            try:
                ...
                print(f"Save Error {img_id}: {e}", file=sys.stderr)
                torch.cuda.empty_cache()
