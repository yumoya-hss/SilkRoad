...
# 用户配置区域
# ==========================================
DEFAULT_QWEN_PATH = "/mnt/raid/hss/model/Qwen3-VL-8B-Instruct"
DEFAULT_SIGLIP_PATH = "/mnt/raid/hss/model/siglip-so400m-patch14-384"
# ✅ 新增：本地 CLIP 模型路径 (请修改为你本地的实际路径)
# 例如: "/mnt/raid/hss/model/CLIP-ViT-H-14-laion2B-s32B-b79K"
DEFAULT_CLIP_PATH = "/mnt/raid/hss/model/CLIP-ViT-H-14" 
DEFAULT_INPUT_DIR = "/mnt/raid/hss/dataset/Image50K"
DEFAULT_FEATURE_PATH = "./siglip_features.pt" # 需确保此文件已由上一步脚本生成
...
def rank_and_score(img_path, img_id, img_features_db, candidates, 
    ...
    """
    ✅ 修改后的评分函数：
    1. 使用 SigLIP 特征进行排序 (Ranking)
    2. 使用 CLIP 模型计算 CLIPScore (Scoring)
    """
    if not candidates: return "", 0.0, 0.0, []
    
    # -------------------------------------------------------
    # 1. SigLIP 排序逻辑 (保持原样，用于选出最佳 Caption)
    # -------------------------------------------------------
    ...
    siglip_probs = [0.0] * len(candidates) # 默认分
    ...
    # -------------------------------------------------------
    # 2. CLIP Score 计算逻辑 (新增)
    # -------------------------------------------------------
    clip_scores = [0.0] * len(candidates)
    try:
        # CLIP 需要读取原图 (因为没有像 SigLIP 那样的预提取特征库)
...
def main():
    parser = argparse.ArgumentParser(description="Step 2: Generate & Rank & CLIPScore")
    ...
    with open(args.output_file, 'a', encoding='utf-8') as f_out:
        for item in tqdm(subset_data, desc=f"GPU{args.gpu_id}"):
            ...
            try:
                # ✅ 排序 (SigLIP) 并打分 (CLIP)
                # 传入 full_img_path 以及 CLIP 相关的模型
