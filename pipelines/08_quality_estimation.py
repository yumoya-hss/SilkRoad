...
# 1. 输入输出
# 上一步回译(04_b)的输出文件
DEFAULT_INPUT_FILE = os.environ.get("SILKROAD_TRANSLATIONS","outputs/translations/translations.json")
# 最终带分数的输出文件
DEFAULT_OUTPUT_FILE = os.environ.get("SILKROAD_SCORED","outputs/scored/scored.json")

# 2. 模型路径
# COMET-Kiwi (SOTA QE模型): 自动下载或指定本地路径
# 如果服务器有网，直接填 "Unbabel/wmt22-cometkiwi-da"
# 离线环境请指向本地路径 (代码会自动寻找 .ckpt)
DEFAULT_COMET_PATH = os.environ.get("SILKROAD_COMET_MODEL","Unbabel/wmt22-comet-da") 

# 🔥 [新增] COMET 底座模型路径 (必须手动下载 infoxlm-large) 🔥
# 用于防止 COMET 尝试联网下载 microsoft/infoxlm-large
DEFAULT_COMET_ENCODER_PATH = os.environ.get("SILKROAD_COMET_ENCODER","xlm-roberta-large")

# CLIP 模型 (用于视觉一致性)
DEFAULT_CLIP_PATH = os.environ.get("SILKROAD_CLIP_MODEL","openai/clip-vit-large-patch14")
